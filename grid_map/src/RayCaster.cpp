...
{
    // From "A Fast Voxel Traversal Algorithm for Ray Tracing" by John Amanatides and Andrew Woo,
    ...
    // The foundation of this algorithm is a parameterized representation of the provided ray,
    // origin + t * direction, except that t is not actually stored; rather, at any given point in
    // the traversal, we keep track of the *greater* t values which we would have if we took a step
    // sufficient to cross a cube boundary along that axis (i.e. change the integer part of the
    // coordinate) in the variables tMaxX, tMaxY, and tMaxZ.
             ...
             const Eigen::Vector3d& max, std::vector<Eigen::Vector3d>* output)
{
    ...
    while (true)
    {
        ...
        // tMaxX stores the t-value at which we cross a cube boundary along the X axis, and
        // similarly for Y and Z. Therefore, choosing the least tMax chooses the closest cube
        // boundary. Only the first case of the four has been commented in detail.
        if (tMaxX < tMaxY)
        {
            if (tMaxX < tMaxZ)
            {
...
bool RayCaster::step(Eigen::Vector3d& ray_pt)
{
    ...
    // tMaxX stores the t-value at which we cross a cube boundary along the X axis, and similarly
    // for Y and Z. Therefore, choosing the least tMax chooses the closest cube boundary. Only the
    // first case of the four has been commented in detail.
    if (tMaxX_ < tMaxY_)
    {
        if (tMaxX_ < tMaxZ_)
        {
